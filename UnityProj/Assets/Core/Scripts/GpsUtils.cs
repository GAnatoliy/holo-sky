...
namespace Assets.Core.Scripts
{
	...
	public class GpsUtils
	{
			...
			out double lat, out double lon)
		{
			...
			var foot = footBar + p2 * Math.Sin(2.0 * footBar) + p4 * Math.Sin(4.0 * footBar) + p6 * Math.Sin(6.0 * footBar) + p8 * Math.Sin(8.0 * footBar) + p10 * Math.Sin(10.0 * footBar);
			var Nf = a / Math.Sqrt(1.0 - ec_sq * Math.Pow(Math.Sin(foot), 2));

			var b1 = 1.0 / (Nf * Math.Cos(foot));
			var b2 = Math.Tan(foot) / (2.0 * Nf * Nf * Math.Cos(foot));
			var b3 = (1.0 + 2.0 * Math.Pow(Math.Tan(foot), 2) + ep_sq * Math.Pow(Math.Cos(foot), 2)) / (6.0 * Math.Pow(Nf, 3) * Math.Cos(foot));
			var b4 = (Math.Tan(foot) * (5.0 + 6.0 * Math.Pow(Math.Tan(foot), 2) + ep_sq * Math.Pow(Math.Cos(foot), 2))) / (24.0 * Math.Pow(Nf, 4) * Math.Cos(foot));
			var b5 = (5.0 + 28.0 * Math.Pow(Math.Tan(foot), 2) + 24.0 * Math.Pow(Math.Tan(foot), 4)) / (120.0 * Math.Pow(Nf, 5) * Math.Cos(foot));
			var d1 = Math.Cos(foot) * (1.0 + ep_sq * Math.Pow(Math.Cos(foot), 2));
			var d2 = -1.0 / 2.0 * Math.Pow(Math.Cos(foot), 2) * Math.Tan(foot) * (1.0 + 4.0 * ep_sq * Math.Pow(Math.Cos(foot), 2));

			var latRadians = -(foot - b2 * d1 * Math.Pow(yWesting, 2) + (b4 * d1 + b2 * b2 * d2) * Math.Pow(yWesting, 4));
			var lonRadians = loMeridianRadians - (b1 * yWesting - b3 * Math.Pow(yWesting, 3) + b5 * Math.Pow(yWesting, 5));
		...
		{
			// longitude of origin
			loMeridian = lon == 0 ? 1 : (int)Math.Truncate(lon / 2.0) * 2 + Math.Sign(lon);    // Take integer part of lon, rounded up to nearest odd number (or down if negative), so =ODD(TRUNC(lon))
			...
			double G = a * (A0 * latRadians + A2 * Math.Sin(2 * latRadians) + A4 * Math.Sin(4 * latRadians) + A6 * Math.Sin(6 * latRadians) + A8 * Math.Sin(8 * latRadians) + A10 * Math.Sin(10 * latRadians));
			double N = a / Math.Sqrt(1.0 - ec_sq * Math.Sin(latRadians) * Math.Sin(latRadians));
			...
			double latTan = Math.Tan(latRadians);
